...
- **Engine** — the local inference server that runs models: Ollama, LM Studio or MAX.
...
default. The scheduler ranks nodes by total pending work across all engines, and
...
- Ollama-compatible and OpenAI-compatible routing proxies, one per engine.
...
- `ollama-proxy`, `lmstudio-proxy` and `max-proxy`: compatible inference proxies
