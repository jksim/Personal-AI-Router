...
- An **engine** is an inference server on a node — Ollama, LM Studio or MAX. A
  node can run any combination, or none, and a node with no running engine is not eligible to
  serve.
...
placed on. Every engine counts together, so load on one affects the ordering
seen by the others.
...
implicit `:latest` tag is normalized; LM Studio and MAX model IDs match exactly
(MAX serves HuggingFace repo ids).
...
**Every engine is counted as one pool.** Load across all engines on a node is
summed, and maximum GPU pressure applies to the whole node. That is conservative on a
...
known install locations for each engine, so an engine you installed yourself is
found where it already is. "Installing" an engine that is already
