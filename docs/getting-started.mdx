...
- **Engines.** PAIR installs and starts Ollama, LM Studio or MAX for you in
  step 4. If an engine is already installed, PAIR detects and uses it instead.
  MAX is Linux x86-64 only; Ollama and LM Studio run on all three platforms.
...
| MAX / OpenAI-compatible proxy | `8000` |
...
| MAX | `http://127.0.0.1:8000` | The OpenAI-compatible paths only |
...
update an engine — engines are updated separately from **Engine
