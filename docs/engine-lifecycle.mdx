...
run models. Today that means Ollama, LM Studio or MAX on a given machine. PAIR
can install and run those engines for you, or work with a copy you already have.
...
Engines differ in where they run. Ollama and LM Studio are available on
Windows, Linux and macOS; **MAX is Linux x86-64 only** and needs an AVX2-class
CPU and glibc 2.34 or newer. PAIR installs MAX into its own self-contained
environment rather than changing the system Python, which costs roughly 1.8 GB
of disk.
...
- If a supported engine is already running on the machine, PAIR can **adopt**
...
| Free that memory without deleting anything | **Eject**, where the engine supports it |
...
### Engines that serve one model at a time

Not every engine holds several models at once. **MAX** serves exactly one model
per process and cannot swap it in place, which changes what two of those actions
mean:

- **Load** selects the model the engine will serve and restarts it onto that
  model. It is a choice about how the engine runs, not an extra model held
  alongside the others. Because MAX compiles a model the first time it loads
  one, this can take minutes rather than seconds.
- **Load** is available while the engine is **stopped**. That is the normal way
  to choose what it will serve the next time it starts.
- There is no **Eject**: the way to free the memory is to stop the engine.
- There is no **Delete**. MAX keeps its models in the shared HuggingFace cache
  (`~/.cache/huggingface`), which other tools on the machine may also be using,
  so PAIR does not delete from it.

A model already in that shared cache is listed whether or not MAX can serve it —
an embedding model put there by another tool will appear, and selecting it fails
because MAX serves text generation. The engine card reports the engine's own
