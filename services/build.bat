...
echo [3/14] Building max-proxy (v%V_MAXPROXY%)...
cd /d "%ROOT%max-proxy"
go build -ldflags "-X main.Version=%V_MAXPROXY%" -o max-proxy.exe . || goto :fail
echo       OK
...
copy /y "%ROOT%max-proxy\max-proxy.exe" "%BIN_OUT%\max-proxy.exe" >nul || goto :fail
...
echo  MAX Proxy:        %BIN_OUT%\max-proxy.exe
