...
// Version reporting: each engine binary stamps its own version via
// -ldflags "-X main.Version=...", which sets that symbol in the BINARY's
// package, not here. So the value is handed to Run and carried on the Proxy —
// a package-level var in this library would never be written by those ldflags
// and every engine would report "dev" forever.
...
// which for streaming responses is dominated by token
...
func (profile Profile) isInferenceRequest(method, path string) bool {
	return method == http.MethodPost && profile.InferenceEndpoints[path]
}
...
type Proxy struct {
	// profile is this proxy's engine identity. Everything else in this type
	// is protocol and cluster behaviour shared by every engine.
	profile Profile

	// version is what this proxy reports in ready notifications.
	version string
...
func NewProxy(codec *Codec, discovery *Discovery, port int, profile Profile) *Proxy {
	return &Proxy{
		profile:   profile,
		version:   "dev",
		codec:     codec,
...
func (p *Proxy) Run(ctx context.Context) error {
	...
	slog.Debug("subscribing to discovery relay for routing targets", "service", string(p.profile.ServiceKey))
	if err := p.codec.Notify(noderec.MethodSubscribe, noderec.SubscribeParams{Services: []noderec.ServiceKey{p.profile.ServiceKey}}); err != nil {
	...
	// connection — so the target engine sees the client disconnect and stops
...
func (p *Proxy) setPort(newPort int) error {
	...
	if err := p.profile.savePersistedPort(newPort); err != nil {
		slog.Warn("failed to persist proxy port", "port", newPort, "err", err)
	}
	if err := p.codec.Notify("ready", ReadyParams{Version: p.version, Port: newPort}); err != nil {
...
func (retrySignal) Error() string { return "openai-proxy: retry next candidate" }
...
// serveModelList queries every candidate concurrently and returns
...
func (p *Proxy) handleHTTP(w http.ResponseWriter, r *http.Request) {
	...
	isInf := p.profile.isInferenceRequest(r.Method, r.URL.Path)
	routingModel := ""
	if isInf {
		routingModel = model
	}
	...
	if r.Method == http.MethodGet && p.profile.ModelListRoutes[r.URL.Path] {
		if len(candidates) > 0 {
...
// (self-forward guard): a local advertisement of this engine could otherwise
...
func (p *Proxy) replaceSubscribed(params json.RawMessage) {
	...
	for _, dn := range res.Nodes {
		n, ok := p.profile.subscribedToNode(dn)
		if !ok {
			continue
		}
	...
	for _, n := range discovered {
		...
		if err := p.codec.Notify("errors:clear", errors.ClearParams{ID: p.profile.upstreamUnreachableID(n.ID)}); err != nil {
	...
	for _, n := range removed {
		...
		if err := p.codec.Notify("errors:report", errors.ServiceError{
			ID:       p.profile.upstreamUnreachableID(n.ID),
...
// for this engine's service key, returning false when the node doesn't
// advertise it or has no dialable address. The engine port comes from that
// service key — the real engine port the broker's poller registered, not the
// proxy's listen port.
func (profile Profile) subscribedToNode(n noderec.DirectoryNode) (Node, bool) {
	svc, ok := n.Services[profile.ServiceKey]
	if !ok || n.IP == "" {
		return Node{}, false
	}
	...
	return Node{
		...
		// Filter on this node's models for this engine only, not the
		// cross-engine union, so a model a multi-engine node serves solely via
		// another engine isn't accepted as an owner here (falls back to the
		// union for a peer that sends no attribution — see EngineModels).
		Models: append([]string(nil), n.EngineModels(profile.Engine)...),
	}, true
}
...
func (p *Proxy) handleMessage(msg *Message) {
	...
	case "set-port":
		...
		if err := p.codec.Respond(msg.ID, ReadyParams{Version: p.version, Port: params.Port}); err != nil {
