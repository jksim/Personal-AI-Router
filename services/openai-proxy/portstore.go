...
func (profile Profile) chooseStartupPort(flagPort int, ignorePersisted bool, persisted int, hasPersisted bool) int {
	if ignorePersisted || !hasPersisted || persisted == profile.LegacyPort {
		return flagPort
	}
	return persisted
}

func (profile Profile) proxyPortPath() (string, error) {
	return appdir.Path(profile.PortFile)
}
...
func (profile Profile) loadPersistedPort() (int, bool) {
	path, err := profile.proxyPortPath()
	if err != nil {
		return 0, false
	}
...
func (profile Profile) savePersistedPort(port int) error {
	path, err := profile.proxyPortPath()
	if err != nil {
		return err
	}
