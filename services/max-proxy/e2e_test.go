...
// proxyBin is the real max-proxy binary, built once in TestMain so the
...
func TestMain(m *testing.M) {
	...
	proxyBin = filepath.Join(tmp, "max-proxy"+suffix)
	if out, err := exec.Command("go", "build", "-o", proxyBin, ".").CombinedOutput(); err != nil {
		panic("build max-proxy: " + err.Error() + "\n" + string(out))
	}
	code := m.Run()
	_ = os.RemoveAll(tmp)
	os.Exit(code)
...
// TestE2EFailoverOverRealBinary spawns the real max-proxy binary and
