...
// ModelLoad warms a model into engine memory using each engine's manifest
// action. An engine that selects its model at launch is routed to SetModel by
// Action, so this path needs no special case.
