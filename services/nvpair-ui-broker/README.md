...
| `max-proxy` | The Modular MAX counterpart, supervised identically | `max-proxy:*` |
...
| `--max-proxy-path <path>` | `./max-proxy[.exe]` in the CWD | Explicit path to the `max-proxy` binary the broker spawns for the local MAX reverse proxy. Same optional semantics as `--proxy-path` |
...
- Path resolution: the explicit `--scanner-path` / `--node-info-path` / `--proxy-path` / `--workload-manager-path` / `--cluster-manager-path` (and the analogous flags for lmstudio-proxy / max-proxy / errors / engine-manager / manual-nodes / settings) if set, otherwise the same-named `./<binary>` (with `.exe` on Windows) in the broker's current working directory. No PATH fallback — a clear "not found" beats a surprising stale binary. A missing scanner is fatal (it's the broker's core job); every other worker is optional.
...
#### `max-proxy:get-status` / `max-proxy:subscribe` / `max-proxy:unsubscribe` / `max-proxy:<method>` (generic relay)

The MAX counterpart of the same surface, running the supervised `max-proxy` on compatibility port `:8000` with the managed MAX backend on `:8001`. It behaves exactly like the LM Studio surface — `get-status` reports the actual bound port, `subscribe` / `unsubscribe` gate the `max-proxy:<event>` stream, any other method is relayed verbatim with the prefix stripped, `max-proxy:shutdown` is refused — with one difference that follows from MAX not necessarily being an engine this node manages.

Ollama and LM Studio are always present in engine-manager's registry. MAX need not be: the router can front a MAX engine some other tool is running, or none at all. On a host where engine-manager has no MAX engine, the broker runs the proxy unmanaged rather than reporting a port-ownership failure — the proxy takes `:8000` itself, its local backend is left cleared, and the ownership gate opens on the port it bound. Nothing waits for a backend port that cannot arrive, and no warning is raised for a configuration that is not wrong. When engine-manager does have a MAX engine, the managed facade behaves identically to LM Studio's.
...
The broker needs `nvpair-node-scanner` reachable as a sibling file in its working directory (or via `--scanner-path`); it also looks for `nvpair-node-info` (or via `--node-info-path`), `ollama-proxy` (or via `--proxy-path`), `nvpair-workload-manager` (or via `--workload-manager-path`), and `nvpair-cluster-manager` (or via `--cluster-manager-path`) the same way (plus lmstudio-proxy / max-proxy / errors / engine-manager / manual-nodes / settings), though all but the scanner are optional. The simplest local layout is to put all the built `.exe`s next to each other — which is exactly how the installer lays them out.
